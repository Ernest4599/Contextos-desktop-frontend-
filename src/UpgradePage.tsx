...
function UpgradePage() {
  ...
  if (codes) {
    return (
      <div className="flex-1 p-6 h-screen overflow-y-auto max-w-2xl mx-auto w-full">
        <h1 className="text-2xl font-bold text-white mb-1">Save your recovery codes</h1>
        ...
        <div className="border border-slate-800 rounded-2xl p-6 mb-6">
          <div className="flex flex-col gap-3">
            {codes.map((c) => (
              <code key={c} className="bg-slate-950 border border-slate-800 rounded-lg px-4 py-3 text-slate-200 text-sm font-mono text-center">
                {c}
              </code>
            ))}
          </div>
        ...
        <label className="flex items-center gap-3 mb-6 cursor-pointer">
          ...
          />
          <span className="text-white text-sm">I've saved my recovery codes</span>
        </label>
  ...
  return (
    <div className="flex-1 p-6 h-screen overflow-y-auto max-w-2xl mx-auto w-full">
      ...
      <h1 className="text-2xl font-bold text-white mb-1">Upgrade ContextOS</h1>
      <p className="text-slate-400 mb-8">Choose a plan to unlock more.</p>
        ...
        {PLANS.map((plan) => (
          <button
            ...
            className={`text-left border rounded-2xl p-5 transition-colors ${
              selectedPlan === plan.id ? 'border-blue-500 bg-blue-600/5' : 'border-slate-800 hover:border-slate-700'
            }`}
          >
