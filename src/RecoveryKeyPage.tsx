...
function RecoveryKeyPage() {
  ...
  return (
    <div className="flex-1 p-6 h-screen overflow-y-auto max-w-2xl mx-auto w-full">
      ...
      <h1 className="text-2xl font-bold text-white mb-1">Recovery Key</h1>
      <p className="text-slate-400 mb-8">Recover a license using one of your saved recovery codes.</p>
      ...
      {!result ? (
        <div className="border border-slate-800 rounded-2xl p-6">
          ...
          <label className="text-white text-sm font-medium block mb-2">Recovery Code</label>
          <input
            type="text"
            value={code}
            onChange={(e) => setCode(e.target.value)}
            placeholder="CTX-XXXX-XXXX-XXXX"
            className="w-full bg-slate-950 border border-slate-700 text-white text-sm rounded-lg px-4 py-3 placeholder:text-slate-500 focus:outline-none focus:border-blue-500 font-mono mb-5"
          />
        ...
        <div className="border border-green-600/40 bg-green-600/10 rounded-2xl p-6 text-center">
          ...
          <h2 className="text-white text-lg font-semibold mb-1">License Recovered</h2>
          <p className="text-slate-300 text-sm mb-1 capitalize">{result.plan} Plan</p>
          <code className="block bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 text-sm font-mono my-4">
            {result.license_key}
          </code>
