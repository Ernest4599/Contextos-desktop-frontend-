  ...
  return (
    <div className="flex-1 p-8 overflow-y-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-white mb-1">Memory</h1>
          <p className="text-slate-400 text-sm">Everything AIOS has stored about you.</p>
        </div>
        <select
          value={category}
          onChange={(e) => {
          ...
          className="bg-slate-900 border border-slate-800 rounded-lg text-sm text-slate-200 px-3 py-2 focus:outline-none focus:border-blue-600"
        >
            ...
            {editingId === m.id ? (
              <div className="flex flex-col gap-3">
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-slate-900 border border-slate-800 rounded-lg p-2 text-sm text-slate-200 resize-none focus:outline-none focus:border-blue-600"
                />
                <div className="flex gap-2 justify-end">
                  <button
                    type="button"
                    onClick={cancelEdit}
                    className="flex items-center gap-1 text-slate-400 hover:text-white text-xs px-3 py-1.5 rounded-lg"
                  >
                    <X size={14} /> Cancel
                  </button>
                  <button
