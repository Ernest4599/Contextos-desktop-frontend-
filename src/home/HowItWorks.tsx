...
function StepCard({ number, title, description, icon }: { number: string; title: string; description: string; icon: React.ReactNode }) {
  return (
    <div className="flex-1 min-w-[clamp(140px,20vw,280px)] rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/60 p-[clamp(10px,1.6vw,24px)]">
      <div className="flex items-start justify-between mb-[clamp(6px,1vw,16px)]">
        ...
        <div className="h-[clamp(14px,1.6vw,24px)] w-[clamp(14px,1.6vw,24px)] text-slate-600 dark:text-slate-500">{icon}</div>
      </div>
      <h3 className="text-[clamp(11px,1.2vw,17px)] font-semibold text-slate-900 dark:text-white mb-[clamp(3px,0.5vw,8px)]">{title}</h3>
      <p className="text-[clamp(9px,0.9vw,13px)] leading-relaxed text-slate-600 dark:text-slate-500">{description}</p>
    </div>
  )
}
